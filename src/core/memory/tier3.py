...
class Tier3Store:
    ...
    All conversation/setting tables carry a ``bot_id`` column so the same
    user can chat with multiple bots in parallel without their histories
    bleeding together. ``bot_id`` defaults to ``"default"`` everywhere so
    pre-multibot callers continue to work; legacy databases are migrated
    in place on :meth:`init`.
    """
    ...
    """
    _CREATE_IDX = """
    CREATE INDEX IF NOT EXISTS idx_turns_user_channel_bot_ts
    ON turns(user_id, channel, bot_id, ts)
    """
    _CREATE_FTS = """
    ...
    async def init(self) -> None:
        ...
        await self._db.commit()
        # Run migration BEFORE creating bot_id-aware indexes so legacy DBs get
        # the column added first.
    ...
    async def _writer_loop(self) -> None:
        while True:
            ...
            if item is None:
                break
            user_id, channel, bot_id, role, content, ts, done_event = item
            try:
                await self._db.execute(
                    "INSERT INTO turns(user_id, channel, bot_id, role, content, ts) VALUES (?,?,?,?,?,?)",
                    (user_id, channel, bot_id, role, content, ts),
                )
                await self._db.commit()
            except Exception:
    ...
    async def save_turn(
        ...
        await self._write_queue.put((user_id, channel, bot_id, role, content, ts, done_event))
        await done_event.wait()

    async def get_recent(
        self, *, user_id: int, channel: str, n: int, bot_id: str = "default",
    ) -> list[dict[str, Any]]:
        async with self._db.execute(
            """SELECT role, content, ts FROM turns
               WHERE user_id=? AND channel=? AND bot_id=?
               ORDER BY id DESC LIMIT ?""",
            (user_id, channel, bot_id, n),
        ) as cur:
    ...
    async def search(
        ...
        async with self._db.execute(
            """SELECT t.role, t.content, t.ts
               ...
               WHERE turns_fts MATCH ? AND t.user_id = ? AND t.channel = ? AND t.bot_id = ?
               ORDER BY rank LIMIT ?""",
            (query, user_id, channel, bot_id, limit),
        ) as cur:
            rows = await cur.fetchall()
        return [{"role": r["role"], "content": r["content"], "ts": r["ts"]} for r in rows]

    async def count_turns(self, *, user_id: int, channel: str, bot_id: str = "default") -> int:
        async with self._db.execute(
            "SELECT COUNT(*) FROM turns WHERE user_id=? AND channel=? AND bot_id=?",
            (user_id, channel, bot_id),
        ) as cur:
    ...
    async def prune_before_id(
        self, *, user_id: int, channel: str, before_id: int, bot_id: str = "default",
    ) -> int:
        async with self._db.execute(
            "SELECT COUNT(*) FROM turns WHERE user_id=? AND channel=? AND bot_id=? AND id<=?",
            (user_id, channel, bot_id, before_id),
        ) as cur:
            row = await cur.fetchone()
        count = row[0] if row else 0
        await self._db.execute(
            "DELETE FROM turns WHERE user_id=? AND channel=? AND bot_id=? AND id<=?",
            (user_id, channel, bot_id, before_id),
        )
        await self._db.commit()
        return count
    ...
    async def get_setting(
        self, *, user_id: int, channel: str, key: str, bot_id: str = "default",
    ) -> str | None:
        async with self._db.execute(
            "SELECT value FROM settings WHERE user_id=? AND channel=? AND bot_id=? AND key=?",
            (user_id, channel, bot_id, key),
        ) as cur:
            row = await cur.fetchone()
    ...
    async def set_setting(
        ...
        await self._db.execute(
            """INSERT INTO settings(user_id, channel, bot_id, key, value)
               VALUES (?,?,?,?,?)
               ON CONFLICT(user_id, channel, bot_id, key) DO UPDATE SET value=excluded.value""",
            (user_id, channel, bot_id, key, value),
        )
        await self._db.commit()
    ...
    async def get_active_role(
        ...
        return (await self.get_setting(
            user_id=user_id, channel=channel, bot_id=bot_id, key="active_role"
        )) or ""
    ...
    async def set_active_role(
        ...
        await self.set_setting(
            user_id=user_id, channel=channel, bot_id=bot_id,
    ...
    async def get_voice_enabled(
        ...
        val = await self.get_setting(
            user_id=user_id, channel=channel, bot_id=bot_id, key="voice_enabled"
        )
        return val == "true"
    ...
    async def get_last_distill_ts(
        ...
        async with self._db.execute(
            "SELECT value FROM settings WHERE user_id=? AND channel=? AND bot_id=? AND key='last_distill_ts'",
            (user_id, channel, bot_id),
        ) as cur:
            row = await cur.fetchone()
        if row is None:
    ...
    async def set_last_distill_ts(
        self, *, user_id: int, channel: str, ts: datetime, bot_id: str = "default",
    ) -> None:
        await self._db.execute(
            """INSERT INTO settings(user_id, channel, bot_id, key, value)
               VALUES (?,?,?,'last_distill_ts',?)
               ON CONFLICT(user_id, channel, bot_id, key) DO UPDATE SET value=excluded.value""",
            (user_id, channel, bot_id, ts.isoformat()),
        )
        await self._db.commit()
    ...
    async def log_usage(
        ...
        await self._db.execute(
            """INSERT INTO usage_logs(user_id, channel, bot_id, runner, prompt_tokens,
               completion_tokens, total_tokens, ts) VALUES (?,?,?,?,?,?,?,?)""",
            (user_id, channel, bot_id, runner, prompt_tokens, completion_tokens, total, ts),
        )
        await self._db.commit()
