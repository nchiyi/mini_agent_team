...
完整的情境化設定 + 操作教學在 [`docs/usage-scenarios/`](docs/usage-scenarios/README.md)：

- [情境 1：DM 1:1 對話](docs/usage-scenarios/01-dm-1on1.md) — 最簡單，單 bot 私訊
- [情境 2：群組 1:多 bot](docs/usage-scenarios/02-group-multibot.md) — 多隻 bot 並列獨立、`@mention` 路由
- [情境 3：Bot 接力工作](docs/usage-scenarios/03-bot-relay.md) — bot 之間 `@mention` 託付、turn-cap 兜底
- [情境 4：多 bot 共同研究](docs/usage-scenarios/04-collaborative-research.md) — `@all` 廣播 + 綜合
- [情境 5：多 bot 辯論觀點](docs/usage-scenarios/05-debate.md) — 每隻 bot 不同立場 role

每篇含可貼上的 `config/config.toml` 片段、Telegram / Discord 雙 channel 步驟、Mermaid 架構圖與訊息流程圖。
